...
namespace WindowsForms
{
    public partial class WorkEssense : Form
    {
        ...
        public WorkEssense(EssenseController essControl)
        {
            InitializeComponent();
                        ...
                        case "ComboBox":
                            {
                                ...
                                cntrl.Location = new Point(200, 25 * yPosControl);
                                #region Попробывать по-другому
                                //var yyy = prop.Name;
                                //object zzz = null;
                                //foreach (var propDb in db.GetType().GetProperties())
                                //{
                                //    if (propDb.PropertyType.DeclaringType.Name==yyy)
                                //    {
                                //        zzz = propDb.GetValue(db);
                                //        break;
                                //    }
                                //}
                                #endregion
                                object collection = essControl.db.GetType().GetProperty(attr.NameClass).GetValue(essControl.db);
                                var x = new List<object>();

                                foreach (var item in (IEnumerable)collection)
                                {
                                    //cntrl.Items.Add(item);
                                    x.Add(item);
                                }
                                cntrl.DataSource = x;
                                propControlsData.Add(prop, cntrl);
        ...
        private void NewFileDialog(object sender, EventArgs e, EssenseController essControl, PropertyInfo prop)
        {
